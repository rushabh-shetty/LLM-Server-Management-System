...
def render_upgrade_tab():
    ...
    with col1:
        st.success("✅ sections_config.xlsx") if "sections" in st.session_state else st.error("❌ No system data yet")
    with col2:
        st.success("✅ Data collected") if "sections" in st.session_state else st.warning("Run Data tab first")
    ...
    if st.button("Run Upgrade Analysis", type="primary", disabled=run_disabled, width='stretch'):
        ...
        with st.status("Running upgrade analysis...", expanded=True) as status:
            ...
            analysis, recommendations, full_raw, full_response = perform_upgrade_analysis(
    ...
    if "last_upgrade" in st.session_state:
        ...
        st.markdown(last["full_response"])
        ...
        with col1:
            if st.button("🔄 Regenerate", width='stretch'):
                ...
                st.rerun()
        with col3:
            report_text = last["full_response"]
            st.download_button(
